...
namespace Test
{
ImplementClass(Test::DatasetTest, 'dstt', Test::TestCase);
...
DatasetTest::Run()
{
    ...
    const URI uri("home:datasettest.db3");
    ...
    this->Verify(db->HasTable("Persons"));
    this->Verify(db->HasTable("Orders"));
    this->Verify(db->HasTable("Products"));
    this->Verify(db->HasTable("Cities"));
    ...
    this->Verify(results->GetNumRows() == 2);
    this->Verify(results->GetString(Attr::Name, 0) == "Franz Jaeger");    
    this->Verify(results->GetString(Attr::Name, 1) == "Franz August");    
