...
DistributedToolkitApp::ParseCmdLineArgs()
{
    ...
    // if a custom project dir arg is given, change the registry key to that dir
    if(this->args.HasArg("-projectdir"))
    {
        if (Win32Registry::Exists(Win32Registry::CurrentUser, "Software\\RadonLabs\\Toolkit", "project"))
        {
            if(!Win32Registry::WriteString(Win32Registry::CurrentUser, "Software\\RadonLabs\\Toolkit", "project",this->args.GetString("-projectdir")))
            {
                return false;
            }
        }
...
DistributedToolkitApp::CreateAndProcessJobs()
{
    ...
    for (jobIndex = 0; jobIndex < jobCount; jobIndex++)
    {
        ...
        String appPath;
        appPath.Append("home:bin/win32/");
        appPath.Append(exeName);
        URI appUri(appPath);
