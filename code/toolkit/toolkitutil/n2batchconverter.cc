...
N2BatchConverter::CheckRequiredProjectInfoAttrs(const ProjectInfo& projInfo)
{
    ...
    if (projInfo.GetCurrentPlatform() == Platform::PS3)
    {
        if (!projInfo.HasAttr("PS3NvdxtTool")) return false;
    }
    return true;
}
...
N2BatchConverter::Setup(Logger& logger, const ToolkitUtil::ProjectInfo& projInfo)
{
    ...
    {
        this->texConverter.SetPS3NvdxtPath(projInfo.GetPathAttr("PS3NvdxtTool"));
    }
