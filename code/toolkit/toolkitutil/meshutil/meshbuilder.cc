...
MeshBuilder::Inflate()
{
    ...
    for (triangleIndex = 0; triangleIndex < numTriangles; triangleIndex++)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            newVertexArray.Append(this->VertexAt(tri.vertexIndex[i]));
...
MeshBuilder::MoveTriangleUvsIntoRange(IndexT triIndex, float minUv, float maxUv)
{
    ...
    for (uvLayerIndex = 0; uvLayerIndex < 4; uvLayerIndex++)
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            maxQuadrantX = max(xQuadrant, maxQuadrantX);
            minQuadrantX = min(xQuadrant, minQuadrantX);
            maxQuadrantY = max(yQuadrant, maxQuadrantY);
            minQuadrantY = min(yQuadrant, minQuadrantY);
        }
