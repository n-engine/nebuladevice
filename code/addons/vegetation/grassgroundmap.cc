...
GrassGroundMap::Setup()
{
    ...
    const Ptr<FramePass>& framepass = frameshader->GetFramePassByName(ResourceId("GrassPatchMapPass"));
    const Ptr<RenderTarget>& renderTarget = framepass->GetRenderTarget();
    this->patchMapSurface = renderTarget->GetCPUResolveTexture();

    const Ptr<FramePass>& framepass1 = frameshader->GetFramePassByName(ResourceId("GrassColorLightNormalWeightMapPass"));
