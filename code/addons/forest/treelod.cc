...
TreeLOD::Setup(float minDist, float maxDist, const Ptr<ModelNode>& node, const StringAtom& nodePath, bool billboard, IndexT lodLevel)
{
    ...
    {        
        ...
        this->billboardShapeNode = child.downcast<ShapeNode>();
        ...
        // extract the billboard geometry (will be written dynamically)
        //this->ExtractBillboardGeometry();

        // setup a shapenode for rendering the combined billboards of this type
        //this->SetupBillboardShapeNode();
    }
}
...
TreeLOD::UpdateInstanceData(const matrix44& tform, float fadeAlpha, float scale)
{
    ...
    /*
    const vector3& pos = tform.pos_component();
    vector3 dir = tform.z_component();
    dir.norm();
