...
TreeClusterPool::Setup(const bbox& levelBoundingBox)
{
    ...
    for (nodeIndex = 0; nodeIndex < numNodes; nodeIndex++)
    {
        ...
        const Ptr<InternalStage>& stage = InternalGraphicsServer::Instance()->GetDefaultView()->GetStage();
        stage->AttachEntity(graphicsEntity.upcast<InternalGraphicsEntity>());
...
TreeClusterPool::Discard()
{
    ...
    {
        const Ptr<InternalStage>& stage = InternalGraphicsServer::Instance()->GetDefaultView()->GetStage();
        stage->RemoveEntity(this->treeClusterGraphicsEntities[i].upcast<InternalGraphicsEntity>());
    }
