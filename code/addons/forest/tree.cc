...
Tree::SetupLevelOfDetails()
{
    ...
    n_assert(root->GetChildren().Size() == 1);

    const Ptr<ModelNode> lodGroupNode = root->GetChildren()[0];
    n_assert(lodGroupNode.isvalid());
    n_assert(lodGroupNode->IsA(TransformNode::RTTI));
    n_assert(lodGroupNode->GetChildren().Size() > 0);

    // combine node path for lod objects
    String nodePath;
    nodePath.Format("%s/%s/", root->GetName().Value(), lodGroupNode->GetName().Value());

    // all child nodes of a LOD Group are lod root nodes
    const Array<Ptr<ModelNode> >& childNodes = lodGroupNode->GetChildren();
