...
/*
void
TreeInstance::PrepareRenderContext(nRenderContext& rc, 
                                   nTransformNode* sceneNode, 
                                   const nArray<nRenderContext*>& lightLinks, 
                                   float fadeAlpha,
                                   const vector4& pointOfInterest)
{
    n_assert(0 != sceneNode);
    rc.SetRootNode(sceneNode);
    rc.SetTransform(this->transform);
    rc.SetFrameId(MGraphics::Server::Instance()->GetFrameId());
    rc.SetLinks(lightLinks);
    rc.SetGlobalBox(this->globalBox);
    rc.GetShaderOverrides().SetArg(nShaderState::Intensity1, fadeAlpha);
    rc.GetShaderOverrides().SetArg(nShaderState::PointOfInterest, pointOfInterest);
    rc.SetFlag(nRenderContext::DoOcclusionQuery, false);
}
*/
//------------------------------------------------------------------------------
/**
