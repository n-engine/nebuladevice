...
*/
float4
VSMPassPixelShader(vsVSMDepthPassOutput psIn) : COLOR
{
    // HOTFIX: cut anything under ground
    if (psIn.depth.y > 0)    
    {
...
AlphaBlendSkinnedColorPassPixelShader(const vsColorPassOutput psIn, uniform int numLights) : COLOR
{
    ...
    float4 color = psLightRim(numLights, psIn.worldPos, psIn.projPos, worldNormal, psIn.worldEyeVec, matDiffuse, matSpecular, MatSpecularPower, matEmissive);
    color = psFog(psIn.projPos.z, color); 
    color.a = matDiffuse.a; // * alphaBlendFactor; on neb2 alphaBlendFactor(Intensity0) is used for RIMPOWER
    // premultiplied alpha
    color.rgb *= color.a;   
    color = EncodeHDR(color); 
    return color;
...
SimpleTechnique(VSMDepth, "VSMDepth", VSMDepthPassVertexShader, VSMPassPixelShader);
...
SimpleTechniqueNoCulling(SkinnedVSMDepth, "VSMDepth|Skinned", SkinnedVSMDepthPassVertexShader, VSMPassPixelShader);
